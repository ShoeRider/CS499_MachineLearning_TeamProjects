...
matpoints(x = dot.x,
          ...
          pch = 19)
```
With a step.size of :**`r Scalar.Step **,and **`r n.hidden.units ** number of n.hidden.units, the optimal number of steps found was: **`r select.step`**. by tweaking these values, a more optimal solution might be possible.
...
lines(colMeans(as.matrix(train.pred.mat)),type="o", col = "red")
```
For the above graph "NNetEarlyStoppingCV SAheart Loss", the Blue line is the validation set error, and Redline is the Test set error. With a step.size of :**`r step.size **,and **`r n.hidden.units ** number of n.hidden.units, the optimal number of steps found was: **`r select.step`**. by tweaking these values, a more optimal solution might be possible.
