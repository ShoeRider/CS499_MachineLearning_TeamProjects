...
Find_Wmatrix_LogisticL2Error<-function(TestingData,TestingLables,W.Matrix,BinaryClassification)
{
  L2Error.matrix = 0

  Y.hat = 1/(1+exp(-data.matrix(TestingData) %*% data.matrix(W.Matrix)))
...
LMLogisticLossIterations<-function(TrainingData, TrainingLabels,Iterations,StepSize.Scalar)
{
  ...
  print(Find_Wmatrix_LogisticL2Error(Normalized_TrainingData,TrainingLabels,t(W.Matrix),BinaryClassification))
...
#
LMLogisticLossEarlyStoppingCV<-function(TrainingData, TrainingLabels, fold.vec,folds.n=4,max.iterations)
{
  step.size = 2
  BinaryClassification =  all(TrainingLabels <= 1 & TrainingLabels >= 0)
  ...
  for(i in 1:folds.n){
    ...
    W.Matrix         =  LMLogisticLossIterations(train.Data,train.Labels,max.iterations,step.size)

    #on the train data, then compute the validation loss of each model.
    Error            =  Find_Wmatrix_LogisticL2Error(test.Data,test.Labels,t(W.Matrix),BinaryClassification)
  ...
  MainWeight.Vector = LMLogisticLossIterations(TrainingData, TrainingLabels,selected.steps,step.size)
  ...
  ReturnList = 0
  #[mean.validation.loss,selected.steps,MainWeight.Vector,function(Feature.Matrix){return(1/(1+exp(-data.matrix(Feature.Matrix) %*% data.matrix(MainWeight.Vector)))) }]
}
...
LMLogisticLossL2CV<-function(TrainingData, TrainingLabels, fold.vec, penalty.vec)
{
  ...
  for(i in 1:folds.n)
  {
    ...
    W.Vector = LMLogisticLossL2penalties(trainData[,1:DataColsEnd], trainData[,LabelCol],penalty.vec)
    ...
    L2Error.Vector = Find_Wmatrix_LogisticL2Error(W.Vector,testData[,1:DataColsEnd],testData[,LabelCol],BinaryClassification)
...
Logistic_Spam_Test<-function()
{
  ...
  #LMLogisticLossIterations(Cliped[,DataColsStart:DataColsEnd], Cliped[,LabelCol],20,0.1)
  max.iterations = 10
  LMLogisticLossEarlyStoppingCV(Cliped[,DataColsStart:DataColsEnd], Cliped[,LabelCol], fold.vec,Folds,max.iterations)
